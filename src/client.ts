...
import OutputWriter from './ui/output-writer'
...
export default class Client {
  ...
  enqueue(code: string, outputEl: HTMLElement, callback: () => void) {
    const msgId = uuidv4();
  ...
  work() {
    if(!this.currentJob) {
      ...
      this.ws.send(`${this.sessionId}/channels,${this.currentJob}`);
    }
  }
  ...
  async handleReply(msg: any) {
    ...
    if (msgType == 'execute_reply') {
      ...
      // Continue working if there are more jobs.
